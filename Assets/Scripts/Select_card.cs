...
public class Select_card : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        //next_pos = cardpos[0].transform.position - transform.position;
        //next_pos = next_pos.normalized;

        //my_rigid.MovePosition(transform.position + next_pos * 5 * Time.fixedDeltaTime);
        //transform.position = Vector3.MoveTowards(transform.position, cardpos[0].transform.position, 5 * Time.fixedDeltaTime);
